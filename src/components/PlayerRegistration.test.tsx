...
describe("PlayerRegistration", () => {
  ...
  describe("duplicate name validation", () => {
    it("should show error when two players have the same name (case-insensitive)", async () => {
      ...
      const submitButton = screen.getByRole("button", {
        name: /Starta spelet/i,
      });
      fireEvent.click(submitButton);

      await waitFor(() => {
        expect(
          screen.getByText("Alla spelarnamn måste vara unika")
        ).toBeInTheDocument();
      });
  ...
  describe("minimum player validation", () => {
    it("should show error when less than 2 players are filled in", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Minst 2 spelare måste fyllas i")
        ).toBeInTheDocument();
      });
      expect(mockOnStartGame).not.toHaveBeenCalled();
    });
  ...
  describe("age selection", () => {
    it("should have default age of 20 for all players", () => {
      ...
      const ageSelect2 = screen.getByLabelText(
        "Ålder för spelare 2"
      ) as HTMLSelectElement;
    ...
    it("should have ages from 0 to 120", () => {
      ...
      const options = Array.from(ageSelect1.options).map((opt) =>
        parseInt(opt.value)
      );

      expect(options.length).toBe(116);
      expect(Math.min(...options)).toBe(5);
